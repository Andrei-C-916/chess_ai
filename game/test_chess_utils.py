...
from chess_utils import board_to_tensor, generate_all_moves

def test_board_to_tensor_shape_and_dtype():
    board = chess.Board()
    tensor = board_to_tensor(board)
    assert isinstance(tensor, np.ndarray)
    assert tensor.shape == (13, 8, 8)

def test_board_to_tensor_legal_moves_plane():
    board = chess.Board()
    tensor = board_to_tensor(board)
...
def test_board_to_tensor_current_player_and_opponent_planes():
    board = chess.Board()
